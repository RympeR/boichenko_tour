...
SELECT * FROM Evidence WHERE DescropyionOfEvidence ~*'((не)*ч(е|ё)тк(ий|ая|ое|ому|ого|ие|им|ой))'
//
...
SELECT CaseId FROM Cases ca
WHERE NOT EXISTS(SELECT ca.caseid FROM ListOfSuspects ls WHERE ca.CaseId = ls.CaseId)
//